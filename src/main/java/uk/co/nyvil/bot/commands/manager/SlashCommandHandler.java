        ...
        if (event.getMember() == null) return;
            ...
            Bot.getInstance().getCommandHandler().getSlashCommandMap().forEach((commandAnnotation, command) -> {
                if (commandAnnotation.name().equalsIgnoreCase(event.getName())) {
                    if(event.getMember().hasPermission(command.neededPermission())) {
                        ...
                        event.replyEmbeds(MessageUtils.createErrorEmbed("You don't have permission to use this command. Needed permission: " + command.neededPermission().getName()).build()).queue();
                    }
